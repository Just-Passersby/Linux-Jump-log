...
# 結論
本來以為用前面的方法可以避免調用RTC，從而讓Linux可以比較容易的調整時區，結果寫紀錄時輸入了`timedatectl`還是`yes`，主要原因在為使用了`--localtime`這個參數，而為何Linux需要使用RTC，可以參考[hwclock](https://man7.org/linux/man-pages/man8/hwclock.8.html)的「LOCAL vs UTC」
