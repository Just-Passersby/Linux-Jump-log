...
Timeshift是Linux用於建立系統還原點的套件，用起來就像是界面陽春點的Time Machine，如果Root FS非BTRFS，那Timeshift幾乎是唯一選擇，如果是Ubuntu/Mint Linux，那預設的佈局也很適合Timeshift。
...
接下來是系統排程，預設是每日，考量到部會每天更新系統，所以設為每週，並把快照數量減少為2，避免太多快照導致檔案不能被釋放<br/>
...
# Timeshift的侷限
## 備份理念
由於其設計理念上更偏向於MacOS的Timemachine，所以TimeShift特別強調和建議快照要用Rsync模式放在其他硬碟，避免系統碟損壞導致無法恢復。<br/>
